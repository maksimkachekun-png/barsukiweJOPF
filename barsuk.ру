...
async def create_invoice(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            f"Сумма: {amount}₽ (~{invoice['amount']} USDT)\n\n"
...
async def connect_group(message: Message, session: AsyncSession):
    ...
    await message.answer(
        f"✅ Группа подключена!\n\n"
        f"Участники покупают через /buy\n"
        f"Деньги списываются с вашего баланса.\n\n"
        f"💰 Ваш баланс: {bal:.2f}₽"
    )
            ...
            for _ in range(30):
                ...
                if check.get('status') == 'RECEIVED':
                    ...
                    await callback.message.edit_text(
                        ...
                        f"💬 Код: {sms_code}\n"
                        f"💰 Списано с владельца: {price_rub}₽"
                    )
                    await callback.answer()
                    return
    ...
    if ticket:
        ...
        try:
            await message.bot.send_message(
                ticket.user_id,
                f"📨 Ответ на обращение #{ticket.id}:\n\n{message.text}"
            )
        except:
            pass
